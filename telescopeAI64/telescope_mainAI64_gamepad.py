...
anglemanualmovespeed=1 #degrees/sec - this is the default speed setting
time_offset=0.0
...
x_button=False
a_button=False
bumperl_button=False
bumperr_button=False
...
# turn base a few times and normalize mag sensor readings
    ...
    a1pulselength=int((1/6)*Axis1_deg_per_step*pruclockfreq) #set frequency
    ...
    while time.monotonic() - start_time < 70.0:
...
def ALT_home(pru0_command_bits):
        ...
        angletohome=math.degrees(math.atan2(round(Accz,2),-round(Accx,2)))-2.70
...
def panright(message):
    session['receive_count'] = session.get('receive_count', 0) + 1
    global anglemanualmovespeed # access to global default stepper speed
    ...
    movespeed=anglemanualmovespeed
    anglemanualmove=moveangle
    if mode=="m":  #if manual mode then move the turn base at the default speed x degrees
        ...
        if ((bitmagic(command_bits,9,"getbit")==0) and (bitmagic(command_bits,10,"getbit")==0) and (bitmagic(command_bits,11,"getbit")==0) ): #make sure the pru is not already running a motor
            command_bits = telescope_motion.AZ_rotate(command_bits,-moveangle,anglemanualmovespeed,0)
...
def altup(message):
    ...
    if mode=="m": #if manual mode then move the turn base at the default speed x degrees
        print (moveangle, " ", movespeed)
        if ((bitmagic(command_bits,9,"getbit")==0) and (bitmagic(command_bits,10,"getbit")==0) and (bitmagic(command_bits,11,"getbit")==0) ):
            command_bits = telescope_motion.ALT_move(command_bits,-moveangle,anglemanualmovespeed,0)
...
def turning_rate_event(message):
    session['receive_count'] = session.get('receive_count', 0) + 1
    global anglemanualmovespeed
    anglemanualmovespeed=float(message['data'])
    emit('my_response',
         {'data': 'speed set', 'count': session['receive_count']},
         broadcast=True)

@socketio.on('time_offset_event')  #traget future position
def time_offset_event(message):
    session['receive_count'] = session.get('receive_count', 0) + 1
    global time_offset
    time_offset=float(message['data'])
    emit('my_response',
         {'data': 'time offset', 'count': session['receive_count']},
         broadcast=True)
...
def Display_data():  # send telescope data to web page
    ...
    while True:
        ...
        tube_sensor_str=str("tube angle: Acceleration (m/s^2): X=%0.5f Y=%0.5f Z=%0.5f"%accel.acceleration+"...Tube angle from vertical:%0.5f"%(angletohome-2.70))
...
def game_pad(): #input
    ...
    global x_button
    global a_button
    global bumperl_button
    global bumperr_button
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            for joystick in joysticks:
                ...
                if joystick.get_button(0): #a pressed
                    a_button=True 
                if joystick.get_button(1): #b pressed
                    b_button=True 
                if joystick.get_button(2): #x pressed
                    x_button=True     
                if joystick.get_button(3): #y pressed
                    y_button=True 
                if joystick.get_button(4): #bumper left pressed
                    bumperl_button=True     
                if joystick.get_button(5): #bumper rigth pressed
                    bumperr_button=True 
                
            hat_move = joystick.get_hat(0)
            if hat_move[0]==-1: #dpad_left
                dpad_left=True
            if hat_move[0]==1: #dpad_right
                dpad_right=True    
            if hat_move[1]==1: #dpad_up
                dpad_up=True
            if hat_move[1]==-1: #dpad_down
                dpad_down=True    
            ...
            if mode=="M" or mode=="m": #manual mode
                ...
                while manual:
                    ...
                    if ((bitmagic(command_bits,9,"getbit")==0) and (bitmagic(command_bits,10,"getbit")==0) and (bitmagic(command_bits,11,"getbit")==0) ):
                        ...
                        if dpad_up:  
                            Display_cmd("Dpad_up pressed")
                            command_bits = telescope_motion.ALT_move(command_bits,-anglemanualmove,anglemanualmovespeed,0)
                            Alt_angle=Alt_angle+anglemanualmove
                            dpad_up=False
                            
                        if dpad_down:
                            Display_cmd("Dpad_down pressed")
                            command_bits = telescope_motion.ALT_move(command_bits,anglemanualmove,anglemanualmovespeed,0)
                            Alt_angle=Alt_angle-anglemanualmove
                            dpad_down=False
                        
                        if dpad_left: 
                            Display_cmd("dpad_left - pressed")
                            command_bits = telescope_motion.AZ_rotate(command_bits,anglemanualmove,anglemanualmovespeed,0)
                            Az_angle=Az_angle-anglemanualmove
                            dpad_left=False
                            
                        if dpad_right: 
                            Display_cmd("dpad_right pressed")
                            command_bits = telescope_motion.AZ_rotate(command_bits,-anglemanualmove,anglemanualmovespeed,0)
                            Az_angle=Az_angle+anglemanualmove
                            dpad_right=False
                            
                        if y_button: 
                            Display_cmd("y_button - zoom in pressed") 
                            command_bits = telescope_motion.focuser_move(command_bits,-Fmanualmove,Fmanualspeed,0)
                            y_button=False

                        if b_button:
                            Display_cmd("b_button - zoom out pressed") 
                            command_bits = telescope_motion.focuser_move(command_bits,Fmanualmove,Fmanualspeed,0)
                            b_button=False

                        if x_button:
                            anglemanualmovespeed+=.1
                            if anglemanualmovespeed>6:
                                anglemanualmovespeed=6
                            displaystring=("turn rate increased to: "+str(anglemanualmovespeed))
                            Display_cmd(displaystring) 
                            x_button=False

                        if a_button:
                            anglemanualmovespeed-=.1
                            if anglemanualmovespeed<.1:
                                anglemanualmovespeed=.1
                            displaystring=("turn rate decreased to: "+str(anglemanualmovespeed))
                            Display_cmd(displaystring)
                            a_button=False

                        if bumperl_button:
                            if anglemanualmove>10:
                                anglemanualmove-=10
                            elif anglemanualmove>2 and anglemanualmove<=10:
                                anglemanualmove-=1
                            elif anglemanualmove<=2:
                                anglemanualmove-=.1
                            if anglemanualmove<0:
                                anglemanualmove=.1
                                                        
                            displaystring=("turn angle decreased to: "+str(anglemanualmove))
                            Display_cmd(displaystring)
                            bumperl_button=False

                        if bumperr_button:
                            if anglemanualmove<=1:
                                anglemanualmove+=.1
                            elif anglemanualmove>1 and anglemanualmove<=10:
                                anglemanualmove+=1
                            elif anglemanualmove>10:
                                anglemanualmove+=10   
                            displaystring=("turn angle increased to: "+str(anglemanualmove))
                            Display_cmd(displaystring)
                            bumperr_button=False
